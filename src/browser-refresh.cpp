...
auto Browser::refresh_callback() -> void {
    ...
    if(auto tab_ptr = tabs.data.current(); tab_ptr != nullptr && tab_ptr->current() != nullptr) {
        ...
        for(auto i = range[0]; i <= range[1]; ++i) {
            ...
            const auto  area = gawl::Rectangle{{layout[0].a.x, y}, {layout[0].b.x, y + Layout::gallery_contents_height}};
            {
                ...
                gallary_contents_font.draw_fit_rect(*this, area, {1.0, 1.0, 1.0, 1.0}, get_display_string(g).data(), 0, gawl::Align::Left);
            }
    ...
    if(auto tab_ptr = tabs.data.current(); tab_ptr != nullptr) {
        ...
        if(auto g_ptr = tab.current(); g_ptr != nullptr) {
            ...
            if(work != nullptr) {
                ...
                auto info_area = gawl::Rectangle{{layout_type == 0 ? layout[1].a.x : thumbnail_bottom, layout_type == 0 ? thumbnail_bottom : layout[1].a.y}, {layout[1].b.x, layout[1].b.y}};

                work_info_font.draw_wrapped(*this, info_area, 24, Color::white, work_info.str().data(), 0, gawl::Align::Left, gawl::Align::Left);
            }
        }
    }
    ...
    if(static_cast<int>(input_key) != -1) {
        ...
        const auto drawed = input_font.draw_fit_rect(*this, area, {0.8, 0.8, 0.8, 1.0}, text.data(), 0, gawl::Align::Left, gawl::Align::Center, [&](const size_t index, const gawl::Rectangle& rect, gawl::TextRenderCharacterGraphic& /* graphic */) -> bool {
            if(index == input_prompt.size()) {
                input_font.set_char_color({1, 1, 1, 1});
            }
            if(index == (input_cursor + input_prompt.size())) {
                gawl::draw_rect(*this, {{rect.a.x, area.a.y + 5}, {rect.a.x + Layout::input_cursor_size[0], area.b.y - 5}}, {1, 1, 1, 1});
            }
            return false;
        });
