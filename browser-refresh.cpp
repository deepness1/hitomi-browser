...
void Browser::refresh_callback() {
    ...
    if(auto tab_ptr = tabs.data.current(); tab_ptr != nullptr && tab_ptr->current() != nullptr) {
        ...
        for(int64_t i = range[0]; i <= range[1]; ++i) {
            ...
            gawl::Area area = {layout[0][0], y, layout[0][2], y + Layout::gallery_contents_height};
            {
                ...
                if(tab.is_current(g)) {
                    gawl::draw_rect(area, {1, 1, 1, 1});
                    gawl::draw_rect({area[0] + 2, area[1] + 2, area[2] - 2, area[3] - 2}, color);
                } else {
                    gawl::draw_rect(area, color);
                }
                gallary_contents_font.draw_fit_rect(area, {1.0, 1.0, 1.0, 1.0}, get_display_string(g).data(), gawl::Align::left);
            }
            ...
            if(tab.type == TabType::reading) {
                ...
                if(download_progress.data.contains(g)) {
                    ...
                    if(std::all_of(progress.begin(), progress.end(), [](bool v) { return v; })) {
                        gawl::draw_rect({area[0], y[0], area[2], y[1]}, Color::download_complete_dot);
                    } else {
                        double     dotw = gallery_contents_area_size[0] / progress.size();
                        gawl::Area dot  = {area[0], y[0], area[0] + dotw, y[1]};
                        for(auto b : progress) {
                            if(b) {
                                gawl::draw_rect(dot, Color::download_progress_dot);
                            }
                            dot[0] += dotw;
                            dot[2] += dotw;
                        }
        ...
        gawl::Area rect;
        work_info_font.get_rect(rect, page_str.data());
        gawl::Area area = {layout[0][2] - (rect[2] - rect[0]), layout[0][3] - (rect[3] - rect[1]), layout[0][2], layout[0][3]};
        gawl::draw_rect(area, Color::black);
        work_info_font.draw_fit_rect(area, Color::white, page_str.data());
    }
    // preview
    if(auto tab_ptr = tabs.data.current(); tab_ptr != nullptr) {
        ...
        if(auto g_ptr = tab.current(); g_ptr != nullptr) {
            ...
            gawl::draw_rect(layout[1], Color::back);
            auto   thumbnail        = get_thumbnail(id);
            double thumbnail_bottom = layout[1][1];
            if(thumbnail) {
                auto fit_area = gawl::calc_fit_rect(layout[1], thumbnail.get_width(), thumbnail.get_height(), layout_type == 0 ? gawl::Align::center : gawl::Align::left, layout_type == 0 ? gawl::Align::left : gawl::Align::center);
                if(fit_area[layout_type == 0 ? 3 : 2] > Layout::preview_thumbnail_limit_rate * preview_area_size[layout_type == 0 ? 1 : 0]) {
                    ...
                    fit_area    = gawl::calc_fit_rect(fit_area, thumbnail.get_width(), thumbnail.get_height());
                }
                thumbnail.draw_rect(fit_area);
            ...
            if(work != nullptr) {
                ...
                auto draw_block = [&](const char* str) {
                    next_line();
                    work_info_font.draw_fit_rect(info_area, Color::white, str, gawl::Align::left, gawl::Align::left, draw_func);
                };
                work_info_font.draw_fit_rect(info_area, Color::white, date_page.data(), gawl::Align::left, gawl::Align::left, draw_func);
                if(!language.empty()) {
    ...
    for(auto& t : tabs.data) {
        std::string disp      = tabs.data.is_current(t) || (t.type == TabType::search && t.searching) ? t.title : t.type == TabType::search ? "[/]" : t.title.substr(0, 3);
        gawl::Area  font_area = {0, 0};
        tab_font.get_rect(font_area, disp.data());
        ...
        if(tabs.data.is_current(t)) {
            gawl::draw_rect({area[0] - 1, area[1] - 1, area[2] + 1, area[3] + 1}, {1, 1, 1, 1});
        }
        gawl::draw_rect(area, Color::tab[static_cast<uint64_t>(t.type)]);
        tab_font.draw_fit_rect(area, {1, 1, 1, 1}, disp.data());
        x += w + padding;
    }
    ...
    if(static_cast<int>(input_key) != -1) {
        ...
        gawl::draw_rect({area[0], area[1] - 2, area[2], area[3] + 2}, {1, 1, 1, 1});
        gawl::draw_rect(area, Color::input_back);
        auto draw_func = [&](size_t index, gawl::Area const& char_area, gawl::GraphicBase& chara) -> bool {
            if(index < input_prompt.size()) {
                input_font.set_char_color({0.8, 0.8, 0.8, 1.0});
                chara.draw_rect(char_area);
                return true;
            } else if(index == (input_cursor + input_prompt.size())) {
                chara.draw_rect(char_area);
                gawl::draw_rect({char_area[0], area[1] + 5, char_area[0] + Layout::input_cursor_size[0], area[3] - 5}, {1, 1, 1, 1});
                return true;
            }
            return false;
        };
        std::string text   = input_prompt + input_buffer.data();
        auto        drawed = input_font.draw_fit_rect(area, {1, 1, 1, 1}, text.data(), gawl::Align::left, gawl::Align::center, draw_func);
        if(input_buffer.size() == static_cast<size_t>(input_cursor)) {
            gawl::draw_rect({drawed[2], area[1] + 5, drawed[2] + Layout::input_cursor_size[0], area[3] - 5}, {1, 1, 1, 1});
        }
    }
    ...
    do {
        ...
        gawl::draw_rect(area, Color::back);
        gawl::draw_rect({area[0], area[1], area[2], area[1] + 3}, Color::white);
        input_font.draw_fit_rect(area, Color::white, message.data.data());
    } while(0);
}
