...
int GraphicBase::get_width() {
    return width / global->magnification;
}
int GraphicBase::get_height() {
    return height / global->magnification;
}
void GraphicBase::draw(double x, double y) {
    draw_rect({x, y, x + width, y + height});
}
void GraphicBase::draw_rect(Area area) {
    global->magnify(area);
    move_vertices(area);
...
void GraphicBase::draw_fit_rect(Area area) {
    draw_rect(calc_fit_rect(area, width, height));
}
