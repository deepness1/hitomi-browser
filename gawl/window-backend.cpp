...
void WaylandWindow::resize_buffer(int width, int height, int scale) {
    ...
    backend_buffer_resize_callback(bw, bh, buffer_scale);
    tell_event();
}
void WaylandWindow::handle_event() {
    ...
    if(repeated != 0) {
        ...
        for(uint32_t i = 0; i < repeated; ++i) {
            backend_keyboard_callback(key, ButtonState::repeat);
        }
    }
    if(do_refresh.load()) {
        refresh();
...
void WaylandWindow::set_event_driven(bool flag) {
    if(event_driven == flag) return;
    if(event_driven) {
        event_driven = false;
    } else {
        event_driven = true;
...
WaylandWindow::WaylandWindow(Application& app, int initial_window_width, int initial_window_height) : display(app.get_display()), tell_event(app.get_tell_event()) {
    ...
    xdg_toplevel.on_close() = [&]() { backend_close_request_callback(); };
    ...
    keyboard.on_key()         = [&](uint32_t /*unused*/, uint32_t /*unused*/, uint32_t key, wayland::keyboard_key_state state) {
        gawl::ButtonState s = state == wayland::keyboard_key_state::pressed ? gawl::ButtonState::press : gawl::ButtonState::release;
        backend_keyboard_callback(key, s);
    ...
    keyboard.on_leave() = [this](uint32_t /* serial */, wayland::surface_t /* surface */) {
        wait_for_key_repeater_exit();
        backend_keyboard_callback(-1, ButtonState::leave);
    };
    pointer.on_button() = [&](uint32_t /*serial*/, uint32_t /*time*/, uint32_t button, wayland::pointer_button_state state) {
        ...
        backend_click_callback(button, s);
    };
    pointer.on_motion() = [&](uint32_t, double x, double y) {
        backend_pointermove_callback(x, y);
    };
    pointer.on_axis() = [&](uint32_t, wayland::pointer_axis axis, double value) {
        ...
        backend_scroll_callback(w, value);
    };
