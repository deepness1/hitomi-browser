...
std::array<gawl::Area, 2> Browser::calc_layout() {
    ...
    switch(layout_type) {
    case 0:
        gallery_contents_area[0] = window_size[0] * (1.0 - split_rate[layout_type]);
        gallery_contents_area[1] = 0;
        gallery_contents_area[2] = window_size[0];
        gallery_contents_area[3] = window_size[1];
        preview_area[0]          = 0;
        preview_area[1]          = 0;
        preview_area[2]          = gallery_contents_area[0];
        preview_area[3]          = window_size[1];
        break;
    case 1:
        gallery_contents_area[0] = 0;
        gallery_contents_area[1] = 0;
        gallery_contents_area[2] = window_size[0];
        gallery_contents_area[3] = window_size[1] * split_rate[layout_type];
        preview_area[0]          = 0;
        preview_area[1]          = gallery_contents_area[3];
        preview_area[2]          = window_size[0];
        preview_area[3]          = window_size[1];
        break;
    }
...
void Browser::buffer_resize_callback(int /* width */, int /* height */, int /* scale */) {
    adjust_cache();
}
Browser::Browser(gawl::Application& app) : gawl::Window(app), temporary_directory("/tmp/hitomi-browser") {
    tab_font              = gawl::TextRender({"/usr/share/fonts/TTF/CascadiaCode.ttf"}, 24);
    ...
    input_font            = gawl::TextRender({"/usr/share/fonts/TTF/CascadiaCode.ttf"}, 24);
    work_info_font        = gawl::TextRender({"/usr/share/fonts/TTF/CascadiaCode.ttf", "/home/mojyack/documents/fonts/seguiemj.ttf"}, 20);
